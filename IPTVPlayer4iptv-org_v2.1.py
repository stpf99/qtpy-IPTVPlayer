...
class IPTVPlayer(QMainWindow):
    ...
    def toggle_fullscreen(self, event):
        ...
        self.adjust_playlist_width()  # Dostosuj szerokość playlisty

    def adjust_playlist_width(self):
        """Automatycznie dostosowuje szerokość playlisty."""
        if self.is_fullscreen:
            # W trybie pełnoekranowym ustaw playlistę na proporcję okna
            self.playlist_tree.setFixedWidth(int(self.width() * 0.2))  # 20% szerokości ekranu
        else:
            # W trybie normalnym szerokość dostosowana do zawartości
            self.playlist_tree.header().setSectionResizeMode(0, QHeaderView.ResizeToContents)
            self.playlist_tree.setMinimumWidth(200)  # Minimalna szerokość
            self.playlist_tree.setMaximumWidth(400)  # Maksymalna szerokość

    def resizeEvent(self, event):
        """Dostosowanie szerokości playlisty przy zmianie rozmiaru okna."""
        super().resizeEvent(event)
        self.adjust_playlist_width()  # Automatyczne dostosowanie


    def hide_playlist(self):
        """Ukryj playlistę w trybie pełnoekranowym."""
        if self.is_fullscreen:
            self.playlist_tree.hide()

    def show_playlist(self):
        """Pokaż playlistę, gdy nie jesteś w trybie pełnoekranowym."""
        if not self.is_fullscreen:
            self.playlist_tree.show()
        
    def setup_auto_hide_timer(self):
        """Konfiguracja timera do automatycznego ukrywania playlisty."""
        self.auto_hide_timer.timeout.connect(self.hide_playlist)
        self.auto_hide_timer.start(5000)  # Ukryj po 5 sekundach

    def mouseMoveEvent(self, event):
        """Pokazuj playlistę podczas ruchu myszką."""
        if self.is_fullscreen:
            self.show_playlist()
            self.auto_hide_timer.start(5000)  # Restart timera
